...
def test_numerical_threads_can_be_tuned_per_provider():
    docs = render(
        f"api.env.SECRET_KEY={PLACEHOLDER_SECRET}",
        "defaults.numericalThreads=2",
        "providers.pmp.numericalThreads=8",
    )
    for var in THREAD_CAP_VARS:
...
def test_an_explicit_env_var_wins_over_the_numerical_threads_cap():
...
def test_nulling_numerical_threads_leaves_the_backends_unbounded():
    docs = render(f"api.env.SECRET_KEY={PLACEHOLDER_SECRET}", "defaults.numericalThreads=null")
    for var in THREAD_CAP_VARS:
