...
cleanup() {
    if [ "$KEEP_RUNNING" = "1" ]; then
        log "KEEP_RUNNING=1, so leaving release '$RELEASE' installed."
        return
    fi
...
ORCHESTRATOR="deployment/${RELEASE}-climate-ref-aft-orchestrator"
orch() { kubectl exec "$ORCHESTRATOR" -- "$@"; }

log "Waiting for orchestrator to come up..."
kubectl wait "$ORCHESTRATOR" --for=condition=available --timeout=300s

log "Initialising providers (no data download for esmvaltool)..."
orch ref providers setup --skip-data --skip-validate
orch ref providers setup --provider pmp
orch ref providers setup --provider ilamb

log "Fetching sample data..."
orch ref datasets fetch-data --registry sample-data --output-directory /ref/sample-data

log "Ingesting sample CMIP6 + obs4mips..."
orch ref -v datasets ingest --source-type cmip6 /ref/sample-data/CMIP6
orch ref -v datasets ingest --source-type obs4mips /ref/sample-data/obs4REF

log "Running a small solve across all three providers..."
orch ref -v solve --timeout 720 --one-per-provider \
...
API="deployment/${RELEASE}-climate-ref-aft-api"
kubectl rollout restart "$API"
kubectl rollout status "$API" --timeout=300s
